...
/* Canonical Peak 3+3+2 policy representatives.
 * policy0 = CPU0-2, policy3 = CPU3-5 (virtual A55), policy6 = CPU6-7.
 * CPU0-5 still share one physical A55 controller/LUT. */
static unsigned int cpu_c0_rep_cpu = 0;
static unsigned int cpu_c1_rep_cpu = 3;
...
MODULE_PARM_DESC(cpu_c0_rep_cpu, "Representative CPU for Peak policy0 (CPU0-2)");
MODULE_PARM_DESC(cpu_c1_rep_cpu, "Representative CPU for Peak policy3 (CPU3-5 virtual A55)");
MODULE_PARM_DESC(cpu_c2_rep_cpu, "Representative CPU for 2-core A76 policy (CPU6-7)");


/*
 * The addon is intentionally coupled to the proven Peak 3+3+2 driver ABI.
 * Refuse to load against a different policy topology instead of silently
 * patching the wrong cpufreq policy or LUT.
 */
static int validate_peak_3plus3plus2(void)
{
	struct cpufreq_policy *p0 = NULL, *p3 = NULL, *p6 = NULL;
	struct cpufreq_mtk_mirror *c0, *c3, *c6;
	cpumask_t m0, m3, m6;
	int ret = -EINVAL;

	cpumask_clear(&m0);
	cpumask_set_cpu(0, &m0);
	cpumask_set_cpu(1, &m0);
	cpumask_set_cpu(2, &m0);
	cpumask_clear(&m3);
	cpumask_set_cpu(3, &m3);
	cpumask_set_cpu(4, &m3);
	cpumask_set_cpu(5, &m3);
	cpumask_clear(&m6);
	cpumask_set_cpu(6, &m6);
	cpumask_set_cpu(7, &m6);

	p0 = cpufreq_cpu_get(cpu_c0_rep_cpu);
	p3 = cpufreq_cpu_get(cpu_c1_rep_cpu);
	p6 = cpufreq_cpu_get(cpu_c2_rep_cpu);
	if (!p0 || !p3 || !p6)
		goto out;
	if (!p0->driver_data || !p3->driver_data || !p6->driver_data)
		goto out;
	if (!cpumask_equal(p0->related_cpus, &m0) ||
	    !cpumask_equal(p3->related_cpus, &m3) ||
	    !cpumask_equal(p6->related_cpus, &m6))
		goto out;

	c0 = (struct cpufreq_mtk_mirror *)p0->driver_data;
	c3 = (struct cpufreq_mtk_mirror *)p3->driver_data;
	c6 = (struct cpufreq_mtk_mirror *)p6->driver_data;

	/* policy3 must be the virtual half of the physical A55 controller. */
	if (c0->shared != c0 || c3->shared != c0 || c3->virtual_id != 1)
		goto out;
	if (c6->shared != c6 || c6->virtual_id != 0)
		goto out;
	if (c0->table != c3->table || c0->nr_opp != c3->nr_opp)
		goto out;
	if (c0->reg_bases[REG_FREQ_LUT_TABLE] !=
	    c3->reg_bases[REG_FREQ_LUT_TABLE])
		goto out;
...
MODULE_PARM_DESC(cpu_c0_target_khz, "Policy0 CPU0-2 target max frequency KHz (0=stock)");
...
static int __init oc_mt6789_init(void)
{
	...
	ret = validate_peak_3plus3plus2();
	if (ret) {
		pr_err("oc_mt6789: unsupported cpufreq topology; expected Peak 3+3+2 (0-2, 3-5, 6-7)\n");
		return ret;
	}
