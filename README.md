...
## MT6789 peak 2-cluster common A76 scale logical policy split

The stock MT6789 DTB exposes two physical CPUFreq performance domains: CPU0-5 (A55) and CPU6-7 (A76). This build keeps those two hardware domains. CPU0-5 now program their own A55 hardware LUT with the same 15 frequency points used by the A76 policy (2200–725 MHz), while CPU6-7 retain their native A76 LUT. The exposed CPUFreq tables are therefore identical without creating a third hardware domain.

The critical frequency change is now in the A55 hardware LUT itself: the first 15 A55 LUT frequency fields are replaced at driver probe with 2200, 2100, 2000, 1900, 1800, 1700, 1600, 1500, 1400, 1300, 1200, 1100, 900, 800 and 725 MHz. The cpufreq table then maps directly to those hardware indices, so 2200 MHz is a real A55 controller request rather than a logical label.
...
When `VIRTUAL_A55_POLICY_SPLIT=y`, the build stage installs the dual-domain `mediatek-cpufreq-hw` replacement. The stock DTB mapping remains CPU0-5 -> domain 0 and CPU6-7 -> domain 1. The A55 driver rewrites only the frequency field of its hardware LUT before enabling the cpufreq HW engine, then exposes exactly those 15 states. No third performance-domain node is created.
